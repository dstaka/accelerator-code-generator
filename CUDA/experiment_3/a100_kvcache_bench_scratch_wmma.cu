...
__global__ void gemm_pv_optimized(const float* __restrict__ P, const half* __restrict__ V_sel,
    ...
    for (int k = 0; k < L; k += BLOCK_K) {
        ...
        for (int i = ty; i < BLOCK_K; i += 8) {
            for (int j = tx; j < BLOCK_N; j += 16) {
                int row = k + i;
                int col = colBase + j;
                Vs[i][j] = (row < L && col < Dv) ? __half2float(V_sel[(size_t)row * Dv + col]) : 0.0f;
            }
        }
        __syncthreads();
...
int main(int argc, char** argv) {
    ...
    std::printf("A100 KV-cache attention benchmark (v1.1.1 - Warp Softmax)\n");
