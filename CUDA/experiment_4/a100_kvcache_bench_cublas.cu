...
int main(int argc, char** argv) {
    ...
    std::printf("A100 KV-cache attention benchmark (v1.2.0 - cuBLAS + Warp Softmax, V in FP32)\n");
    ...
    if (d == "fp16" || d == "half") {
        ...
        for (int i = 0; i < args.warmup; ++i) {
            ...
            // O = P * V_sel => [M x L] * [L x Dv] => [M x Dv]
            // V_sel is now FP32, no cast needed
        ...
        for (int i = 0; i < args.iters; ++i) {
            ...
            // V_sel is already FP32, no cast needed
            checkCublas(cublasGemmEx(handle, opN, opN, Dv, M, L,
                                     &alpha,
                                     V_sel, CUDA_R_32F, Dv,
                                     P, CUDA_R_32F, L,
...
// Log (v1.2.0 - Fair comparison with V in FP32, no cast overhead)
// ./a100_kvcache_bench_cublas 4096 8192 128 128 50 10 65536 fp16
// A100 KV-cache attention benchmark (v1.2.0 - cuBLAS + Warp Softmax, V in FP32)
